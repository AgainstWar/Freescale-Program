...
void lcd_init_simspi(void)
{
    ...
    {
    case 0:
        lcd_writeData_simspi(1 << 7 | 1 << 6 | 0 << 5);
        break; //竖屏模式
    case 1:
        lcd_writeData_simspi(0 << 7 | 0 << 6 | 0 << 5);
        break; //竖屏模式  旋转180
    case 2:
        lcd_writeData_simspi(1 << 7 | 0 << 6 | 1 << 5);
        break; //横屏模式
    case 3:
        lcd_writeData_simspi(0 << 7 | 1 << 6 | 1 << 5);
        break; //横屏模式  旋转180
    }
...
void lcd_showchar_simspi(uint16 x, uint16 y, const int8 dat)
{
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        {
            if (temp & 0x01)
                lcd_writeData_16bit_simspi(TFT_PENCOLOR);
            else
                lcd_writeData_16bit_simspi(TFT_BGCOLOR);
            temp >>= 1;
        }
    }
}
